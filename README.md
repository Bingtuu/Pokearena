...
- **🏆 真实赛事卡组管线** —— mik 赛事 API 全链路：26 场赛事 / 1,252 套卡组内容 / 1,396 条出战记录入库；卡组内容实体与出战条目分表建模，`mapping_status` 分档只统计可映射简中环境的卡组
...
> Phase 1（M1~M4）+ Phase 2 跨语言映射（M6）+ 赛事与统计基建（M9-1/M9-2）已完成：129 系列 / 12,420 张卡、26 赛 / 1,252 卡组入库，验收 A1~A8 全过，293 测试全绿。以下接口均已可用（`validate_deck` 为 M7 剩余目标，A2/A3 卡面人工比对 M8 进行中）。

**CLI**

```bash
ptcgdb scrape sets && ptcgdb scrape cards      # 采集卡牌（mik.moe 主源，限速 2s/请求）
