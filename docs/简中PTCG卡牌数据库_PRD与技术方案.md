...
- 白名单分两部分（完整清单见附录 A）：
  ...
  - **过去系列卡牌**：标准赛制 26 种，**开放赛制另有更长的 34 种清单**（多出捕虫少年、离洞绳、谜之化石、模仿少女、能量签、千金小姐 等 8 种）[^1^]——白名单必须**按赛制独立**维护。官方明确"卡牌上的描述**按最新卡牌的描述为准**"[^1^]。
...
- FR-5.4 赛季日历：每 2~3 个月赛季开启时强制全量对账（库内合法性判定结果 vs 赛制页**按赛制分别**逐卡核对——标准白名单 26 种 / 开放白名单 34 种各自核对）。
...
| # | 标准 | 度量 |
|---|---|---|
| A1 | 覆盖完整 | G/H/I 全部系列 + 开放赛制涉及系列入库，系列级对账（含 expected_secret_count）100% 通过；白名单逐卡核对无缺（标准：18 特典 + 26 旧卡 + 8 能量；开放：34 旧卡 + 9 能量，分赛制核对） |
...
| A4 | 合法性引擎 | `legal_at('2026-08-01', standard)` 结果与赛制页逐卡一致；构造用例 ≥12 组全部通过（含：博士的研究跨插画、ACE SPEC、各 owner 前缀、**妖能量：standard 不合法 / open 合法**、**视作B 覆盖：天空之柱 CSM2D-339 合法而同名其他印刷不合法**、开放赛制 34 种白名单抽样） |
...
**过去系列卡牌（开放赛制 34 种）**：在标准 26 种基础上多出捕虫少年、离洞绳、谜之化石、模仿少女、能量签、千金小姐 等 8 种（完整清单以赛制页正文及"特别的卡牌"外链为准，L1 监控维护）。
