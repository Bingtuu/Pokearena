...
- 物化视图（v1.10，FR-9.7）：`v_stat_deck_cards`（四表联查[deck_cards ⋈ decks ⋈ deck_appearances ⋈ tournaments] + 统计范围过滤 + group_key 预联，行粒度 = 出战条目 × 卡）、`v_tournament_weights`（赛事静态权重件）随迁移落库、导出 DB 自带；视图只封装过滤与连接，业务公式只在 canonical SQL（`ptcgdb/stats/sql/`）。**basis 口径列（v1.14）**：两视图加 basis 列（source→basis 映射：mik_moe→cn / limitless→intl_aligned / pokemon_card_jp→jp），`--basis` 过滤与 meta 回显均以视图列为准，EN/JP 样本不与 CN 混同（FR-9.1a）。
