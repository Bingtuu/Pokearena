"""FR-3.4 同名计数引擎（task 025，PRD v1.7）。

纯函数核 `check_counts`：操作 frozen Pydantic 模型，供 CLI 与 SDK 双后端复用
（对齐 legal engine 的分层模式；banned/not_legal 由 validate_deck（task 026）
组合 select_snapshot/build_pool 后判定，不在本模块）。
